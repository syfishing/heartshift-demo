...
# True when pause stopped a track that still owes a resume. Distinguishes "the
# menu paused us" from "the song finished" or "it never started".
...
func play_track(audio: AudioStream):
	...
	# The menu can already be open when the track is due to start, because the
	# tutorial starts its level late. Hand the track to the resume rather than
	# playing it underneath the menu.
	if pause_state:
...
func clear_pause_state() -> void:
	# A freshly loaded level is never paused. Levels can be left with the menu
	# still open, so the pause menu clears this when it enters the tree.
	pause_state = false
	_resume_pending = false
...
func toggle_trackpause() -> void:
	if pause_state == false:
		# pause_state means "the game wants music paused", so it is set even when
		# there is no track to stop yet. Only an actually playing track can be
		# captured, and only that owes a resume.
		pause_state = true
		...
		_resume_pending = false
		# Seeking past the end of an Ogg stream silently wraps back to 0.0.
